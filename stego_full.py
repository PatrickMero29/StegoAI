...
def main():
    ...
    try:
        ...
        # Convert 0/1 to -1.0/1.0 and shape it for the Generator (1 batch, 256 length)
        # Note: Your mapping.py pads the remaining space with random decimals automatically
        secret_vector = encoder.binary_to_latent(binary_list, batch_size=1).to(device)
        
        # Keep a copy of the exact math the Generator was fed to compare later
    ...
    # CRITICAL FIX: value_range=(-1.0, 1.0) guarantees the math isn't warped when saving
    ...
    # CRITICAL FIX: Slice [:message_length] so we only score the actual text, not the random padding!
